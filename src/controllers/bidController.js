...
const placeBid = async (req, res) => {
  ...
  try {
    ...
    const [stats] = await connection.query('SELECT * FROM monthly_stats WHERE user_id = ? AND month = ?', [req.user.id, monthStr]);
    ...
    // Check existing bid
    const [existing] = await connection.query('SELECT * FROM bids WHERE user_id = ? AND bid_date = ?', [req.user.id, bidDateStr]);
    
    if (existing.length > 0) {
      connection.release();
      return res.status(400).json({ error: 'Bid already placed for tomorrow. Use update to increase.' });
    }

    await connection.query('INSERT INTO bids (user_id, amount, bid_date, status) VALUES (?, ?, ?, ?)', [req.user.id, amount, bidDateStr, 'PENDING']);
...
const updateBid = async (req, res) => {
  ...
  try {
    ...
    if (amount <= existing[0].amount) {
      connection.release();
      return res.status(400).json({ error: 'New amount must be higher than current amount.' });
    }

    await connection.query('UPDATE bids SET amount = ? WHERE user_id = ? AND bid_date = ?', [amount, req.user.id, bidDateStr]);
    connection.release();
...
const getBidStatus = async (req, res) => {
  ...
  try {
    ...
    const [highest] = await connection.query('SELECT amount FROM bids WHERE bid_date = ? ORDER BY amount DESC LIMIT 1', [bidDateStr]);
...
const getMonthlyStatus = async (req, res) => {
  ...
  try {
    const connection = await pool.getConnection();
    const [stats] = await connection.query('SELECT * FROM monthly_stats WHERE user_id = ? AND month = ?', [req.user.id, currentMonth]);
    connection.release();
