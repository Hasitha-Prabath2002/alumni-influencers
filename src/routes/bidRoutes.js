...
const { placeBid, updateBid, getBidStatus, getMonthlyStatus } = require('../controllers/bidController');
 ...
 */
router.post('/', [
  body('amount').isNumeric().custom(value => value > 0).withMessage('Bid amount must be positive').escape()
], placeBid);
 ...
 */
router.put('/', [
  body('amount').isNumeric().custom(value => value > 0).withMessage('Increase bid correctly').escape()
], updateBid);
