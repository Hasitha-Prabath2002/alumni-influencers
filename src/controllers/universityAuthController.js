  ...
  try {
    ...
    await connection.query(
      `INSERT INTO university_users (email, password, first_name, last_name, verification_token, token_expires)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [email, hashedPassword, firstName, lastName, verificationToken, tokenExpires]
    );
...
const verifyEmail = async (req, res) => {
  ...
  try {
    ...
    connection.release();
    res.json({ message: 'Email verified successfully. You can now log in to the Analytics Dashboard.' });
  } catch (error) {
...
const login = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: user.id, email: user.email, role: user.role, type: 'university_staff' },
