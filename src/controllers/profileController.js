...
const getProfile = async (req, res) => {
  try {
    ...
    const [profile] = await connection.query('SELECT * FROM profiles WHERE user_id = ?', [req.user.id]);
    const [degrees] = await connection.query('SELECT * FROM degrees WHERE user_id = ?', [req.user.id]);
    const [certifications] = await connection.query('SELECT * FROM certifications WHERE user_id = ?', [req.user.id]);
    connection.release();
...
const _generateGetter = (tableName) => async (req, res) => {
  try {
    const connection = await pool.getConnection();
    const [data] = await connection.query(`SELECT * FROM ${tableName} WHERE user_id = ?`, [req.user.id]);
    connection.release();
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: 'Internal Server Error' });
...
const _generateDeleter = (tableName) => async (req, res) => {
  try {
    const connection = await pool.getConnection();
    await connection.query(`DELETE FROM ${tableName} WHERE id = ? AND user_id = ?`, [req.params.id, req.user.id]);
    connection.release();
...
const uploadProfileImage = async (req, res) => {
  ...
  try {
    const connection = await pool.getConnection();
    await connection.query('UPDATE profiles SET profile_image_url = ? WHERE user_id = ?', [imageUrl, req.user.id]);
    connection.release();

    res.json({ message: 'Profile image uploaded successfully', imageUrl });
  } catch (error) {
    res.status(500).json({ error: 'Database error while saving profile image URL' });
  }
};

module.exports = {
  getProfile, updateProfile,
  addDegree: _generateAdder('degrees', ['degreeName', 'universityUrl', 'completionDate']),
  getDegrees: _generateGetter('degrees'),
  deleteDegree: _generateDeleter('degrees'),
  addCertification: _generateAdder('certifications', ['certName', 'courseUrl', 'completionDate']),
  getCertifications: _generateGetter('certifications'),
  deleteCertification: _generateDeleter('certifications'),
  addLicence: _generateAdder('licences', ['licenceName', 'bodyUrl', 'completionDate']),
  getLicences: _generateGetter('licences'),
  deleteLicence: _generateDeleter('licences'),
  addCourse: _generateAdder('courses', ['courseName', 'courseUrl', 'completionDate']),
  getCourses: _generateGetter('courses'),
  deleteCourse: _generateDeleter('courses'),
  addEmployment: _generateAdder('employment', ['companyName', 'role', 'startDate', 'endDate']),
