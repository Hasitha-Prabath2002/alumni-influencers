...
const generateApiKey = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'API Key generated safely store it, it will not be shown again.', apiKey, name });
  } catch (error) {
...
const revokeApiKey = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Key not found or you lack permission' });
...
const getKeyStats = async (req, res) => {
  ...
  try {
    ...
    // Verify ownership
    const [keys] = await connection.query('SELECT * FROM api_keys WHERE id = ? AND user_id = ?', [id, req.user.id]);
    if (keys.length === 0) {
      connection.release();
