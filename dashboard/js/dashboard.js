...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('user-name').textContent = `${user.firstName || ''} ${user.lastName || ''}`.trim() || user.email;
  document.getElementById('user-role').textContent = user.role || 'analyst';
  document.getElementById('user-avatar').textContent = (user.firstName || 'U')[0].toUpperCase();
...
async function navigateTo(section) {
  ...
  const titles = { overview:'Dashboard Overview', charts:'Charts & Analytics', skills:'Skills Gap Intelligence', careers:'Career Pathways', alumni:'View Alumni', usage:'API Usage & Security' };
...
async function loadOverview() {
  ...
  if (overview) {
    ...
    setKPI('kpi-v-featured', f ? `${f.first_name} ${f.last_name}` : 'No winner yet');
  }
  ...
  if (profDev) {
    ...
    renderChart('breakdown', 'doughnut', {
      ...
      datasets: [{ data: [d.certifications, d.courses, d.licences], backgroundColor: ['#6366f1','#06b6d4','#10b981'], borderWidth: 0 }]
    });
  }
  ...
  if (programmes?.programmes?.length) {
    ...
    renderChart('programmes', 'bar', {
      labels: top.map(p => truncate(p.programme, 18)),
      datasets: [{ label: 'Alumni', data: top.map(p => p.alumni_count), backgroundColor: PALETTE, borderRadius: 6 }]
  ...
  if (graduation?.byYear?.length) {
    renderChart('graduation', 'line', {
      ...
      datasets: [{ label: 'Graduates', data: graduation.byYear.map(r => r.alumni_count), borderColor: '#6366f1', backgroundColor: 'rgba(99,102,241,0.1)', fill: true, tension: 0.4, pointRadius: 4 }]
    });
  }
}
...
async function loadChartsSection() {
  ...
  // Line: cert trends
  if (trends) {
    const labels = [...new Set([...trends.certifications.map(r=>r.month), ...trends.courses.map(r=>r.month)])].sort();
    const getCounts = (arr) => labels.map(m => arr.find(r => r.month === m)?.count || 0);
    renderChart('cert-trends', 'line', {
      labels,
      datasets: [
        { label: 'Certifications', data: getCounts(trends.certifications), borderColor: '#6366f1', backgroundColor: 'rgba(99,102,241,0.08)', fill: true, tension: 0.4 },
        { label: 'Courses',        data: getCounts(trends.courses),        borderColor: '#06b6d4', backgroundColor: 'rgba(6,182,212,0.08)',  fill: true, tension: 0.4 },
        { label: 'Licences',       data: getCounts(trends.licences),       borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,0.08)', fill: true, tension: 0.4 }
      ]
  ...
  if (careers?.roles?.length) {
    ...
    renderChart('roles', 'bar', {
      labels: top.map(r => truncate(r.role, 22)),
      datasets: [{ label: 'Alumni', data: top.map(r => r.count), backgroundColor: PALETTE, borderRadius: 4 }]
    }, { indexAxis: 'y', plugins: { legend: { display: false } }, scales: { x: { grid: GRID, beginAtZero: true }, y: { grid: { display: false } } } });
  }
  ...
  if (profDev?.topSkills?.length) {
    ...
    renderChart('skills-bar', 'bar', {
      labels: top.map(s => truncate(s.skill_name, 20)),
      datasets: [{ label: 'Alumni Count', data: top.map(s => s.total_count), backgroundColor: PALETTE.map(c => c + 'cc'), borderColor: PALETTE, borderWidth: 1, borderRadius: 6 }]
    }, { scales: { x: { grid: { display: false } }, y: { grid: GRID, beginAtZero: true } }, plugins: { legend: { display: false } } });
  }

  // Radar: averages
  if (profDev?.averages) {
    const a = profDev.averages;
    renderChart('radar', 'radar', {
      labels: ['Certifications', 'Courses', 'Licences'],
      datasets: [{ label: 'Avg per Alumnus', data: [a.certificationsPerAlumnus, a.coursesPerAlumnus, a.licencesPerAlumnus], borderColor: '#6366f1', backgroundColor: 'rgba(99,102,241,0.2)', pointBackgroundColor: '#6366f1' }]
    }, { scales: { r: { grid: { color: 'rgba(255,255,255,0.06)' }, ticks: { backdropColor: 'transparent' } } } });
  }
}
...
async function loadSkillsGap() {
  ...
  if (data.certifications?.length) {
    ...
    const colors = top.map(s => s.severity === 'critical' ? '#ef4444' : s.severity === 'significant' ? '#f59e0b' : '#10b981');
    renderChart('gap-certs', 'bar', {
      labels: top.map(s => truncate(s.skill_name, 24)),
      datasets: [{ label: '% of Graduates', data: top.map(s => s.percentage), backgroundColor: colors.map(c => c + 'aa'), borderColor: colors, borderWidth: 1, borderRadius: 6 }]
    }, { scales: { x: { grid: { display: false } }, y: { grid: GRID, beginAtZero: true, max: 100 } }, plugins: { legend: { display: false } } });
  }
  ...
  // Table
  const all = [...(data.certifications || []), ...(data.courses || [])].sort((a,b) => b.percentage - a.percentage);
  ...
  tbody.innerHTML = all.map(s => `
    ...
    </tr>`).join('') || '<tr><td colspan="5" class="loading-row">No data</td></tr>';
}
...
async function loadCareers() {
  ...
  if (data.timeline?.length) {
    renderChart('timeline', 'line', {
      labels: data.timeline.map(r => r.year),
      datasets: [{ label: 'Alumni Employed', data: data.timeline.map(r => r.employed_count), borderColor: '#6366f1', backgroundColor: 'rgba(99,102,241,0.15)', fill: true, tension: 0.4, pointRadius: 5 }]
    }, { scales: { x: { grid: GRID }, y: { grid: GRID, beginAtZero: true } } });
  }
  ...
  if (data.companies?.length) {
    ...
    renderChart('employers', 'doughnut', {
      ...
      datasets: [{ data: top.map(c => c.count), backgroundColor: PALETTE, borderWidth: 0 }]
    });
  }
  ...
  if (data.roles?.length) {
    ...
    renderChart('polar', 'polarArea', {
      labels: top.map(r => truncate(r.role, 20)),
      datasets: [{ data: top.map(r => r.count), backgroundColor: PALETTE.map(c => c + '99'), borderColor: PALETTE, borderWidth: 1 }]
    }, { scales: { r: { grid: { color: 'rgba(255,255,255,0.06)' }, ticks: { backdropColor: 'transparent' } } } });
  }
}
...
async function loadAlumni(page = 1) {
  ...
  const params    = new URLSearchParams({ page, limit: 12, ...(programme && { programme }), ...(year && { year }), ...(industry && { industry }) });
  ...
  if (!data?.alumni?.length) {
    grid.innerHTML = '<div class="loading-placeholder">No alumni found for current filters.</div>';
    pgn.innerHTML = '';
    return;
  }

  grid.innerHTML = data.alumni.map(a => {
  ...
  pgn.innerHTML = Array.from({ length: Math.min(pages, 10) }, (_, i) => i + 1)
    .map(p => `<button class="page-btn ${p === page ? 'active' : ''}" onclick="loadAlumni(${p})">${p}</button>`)
    .join('');
}
...
function clearAlumniFilters() {
  ['filter-programme','filter-year','filter-industry'].forEach(id => document.getElementById(id).value = '');
  loadAlumni(1);
}
...
async function exportAlumniCSV() {
  ...
  if (!data?.alumni?.length) { alert('No data to export.'); return; }

  const headers = ['First Name','Last Name','Email','Programme','Graduation','Role','Company'];
  const rows = data.alumni.map(a => [
    a.first_name, a.last_name, a.email, a.programme || '',
...
async function exportReport() {
  ...
  doc.text(`Generated: ${new Date().toLocaleDateString('en-GB', { weekday:'long', year:'numeric', month:'long', day:'numeric' })}`, 20, 38);
  ...
  const kvs = [
    ...
    ['Courses Completed', document.getElementById('kpi-v-courses')?.textContent],
  ...
  doc.setTextColor(241, 245, 249); doc.setFontSize(12); doc.setFont('helvetica','bold');
  doc.text('Key Metrics', 20, 52);
  doc.setFontSize(10); doc.setFont('helvetica','normal');
  kvs.forEach(([k,v], i) => {
    doc.setTextColor(100,116,139); doc.text(k + ':', 20, 62 + i * 8);
    doc.setTextColor(241,245,249); doc.text(v || '—', 80, 62 + i * 8);
  });
...
async function loadUsage() {
  ...
  if (data.dailyRequests?.length) {
    renderChart('daily-requests', 'line', {
      labels: data.dailyRequests.map(r => r.date),
      datasets: [{ label: 'API Requests', data: data.dailyRequests.map(r => r.request_count), borderColor: '#06b6d4', backgroundColor: 'rgba(6,182,212,0.1)', fill: true, tension: 0.4 }]
    }, { scales: { x: { grid: GRID }, y: { grid: GRID, beginAtZero: true } } });
  }

  // Usage table
  const uBody = document.getElementById('usage-table-body');
  uBody.innerHTML = (data.endpointStats || []).map(s => `<tr>
  ...
  kBody.innerHTML = (data.keyUsage || []).map(k => {
...
function saveApiKey() {
  ...
  if (key) { localStorage.setItem('dashboard_api_key', key); location.reload(); }
...
/* ===== LOGOUT ===== */
function logout() { Auth.clearSession(); window.location.href = 'login.html'; }
